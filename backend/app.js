...
//config
const secretKey = 'croco-salma-23-cun';
app.use(session({
    secret: secretKey,
}));
...
app.post("/players", (req, res) => {
    ...
    Team.findById(req.body.team).then((team) => { // Corrected team ID
        if (!team) {
...
app.get('/teams/:id' ,(req,res)=>{
    console.log("here into team id" , req.params.id);
Team.findById(req.params.id).populate()
...
app.get('/users/:id', (req, res) => {
    ...
    User.findById(userId).then(
        (doc) => {
            console.log("here doc", doc);
            res.json({ findUser: doc });
        }
    )
}
)
// business logic to search by city
